...
public class LList2<T> implements ListInterface<T> {
	...
	int getLastIndex(T item) {
	   ...
	   if (item == getNodeAt(numberOfEntries).getData())
		  return numberOfEntries;
	   else {
		  for(int counter=1; counter <=numberOfEntries; counter++){
			 if(item == getNodeAt(counter).getData()){
				index = counter;
			 }
		  }
	   }
	...
	public boolean equals(Object other) {
	   ...
	   if ((other == this) || (this == null && other == null))
	   return true;
	   ...
	   if (this.getLength() != otherList.getLength()) {
		  ...
		  for (int i = 1; i <= otherList.getLength(); i++) {
			 if (this.getNodeAt(i).getData() != otherList.getNodeAt(i).getData()) {
				return false;
			 }
